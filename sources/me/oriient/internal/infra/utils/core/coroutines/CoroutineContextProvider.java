package me.oriient.internal.infra.utils.core.coroutines;

import androidx.annotation.Keep;
import kotlin.Metadata;
import kotlin.coroutines.CoroutineContext;
import me.oriient.internal.infra.utils.core.DiProvidable;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

@Keep
@Metadata(d1 = {"\u0000\u001a\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\b\n\u0002\u0010\b\n\u0002\b\u0004\bg\u0018\u00002\u00020\u0001J\b\u0010\n\u001a\u00020\u0003H&J\u0010\u0010\n\u001a\u00020\u00032\u0006\u0010\u000b\u001a\u00020\fH&J!\u0010\r\u001a\u00020\u00032\u0006\u0010\u000e\u001a\u00020\f2\n\b\u0002\u0010\u000b\u001a\u0004\u0018\u00010\fH&¢\u0006\u0002\u0010\u000fR\u0012\u0010\u0002\u001a\u00020\u0003X¦\u0004¢\u0006\u0006\u001a\u0004\b\u0004\u0010\u0005R\u0012\u0010\u0006\u001a\u00020\u0003X¦\u0004¢\u0006\u0006\u001a\u0004\b\u0007\u0010\u0005R\u0012\u0010\b\u001a\u00020\u0003X¦\u0004¢\u0006\u0006\u001a\u0004\b\t\u0010\u0005¨\u0006\u0010"}, d2 = {"Lme/oriient/internal/infra/utils/core/coroutines/CoroutineContextProvider;", "Lme/oriient/internal/infra/utils/core/DiProvidable;", "default", "Lkotlin/coroutines/CoroutineContext;", "getDefault", "()Lkotlin/coroutines/CoroutineContext;", "io", "getIo", "main", "getMain", "newSingleThreadCoroutineContext", "priority", "", "newThreadPoolCoroutineContext", "numThreads", "(ILjava/lang/Integer;)Lkotlin/coroutines/CoroutineContext;", "internal_publishRelease"}, k = 1, mv = {1, 9, 0}, xi = 48)
/* loaded from: classes7.dex */
public interface CoroutineContextProvider extends DiProvidable {
    @NotNull
    CoroutineContext getDefault();

    @NotNull
    CoroutineContext getIo();

    @NotNull
    CoroutineContext getMain();

    @NotNull
    CoroutineContext newSingleThreadCoroutineContext();

    @NotNull
    CoroutineContext newSingleThreadCoroutineContext(int priority);

    @NotNull
    CoroutineContext newThreadPoolCoroutineContext(int numThreads, @Nullable Integer priority);
}

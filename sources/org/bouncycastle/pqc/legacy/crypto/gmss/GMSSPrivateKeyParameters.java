...
public class GMSSPrivateKeyParameters extends GMSSKeyParameters {
            ...
            for (int i13 = 0; i13 < iHeightOfPhi; i13++) {
                if (i13 < i2) {
                    if (this.currentTreehash[i][i13].wasFinished()) {
                        ...
                        System.err.println(b.j(i, i13, "Treehash (", ",", ") not finished when needed in AuthPathComputation"));
                    }
                }
                if (i13 < i9 && i13 >= i2) {
                    int i14 = i13 - i2;
    ...
    private void updateKey(int i) {
        ...
        if (i > 0) {
            ...
            int iFloor = (int) Math.floor((getNumLeafs(i) * 2) / (this.heightOfTrees[i3] - this.K[i3]));
            int i4 = this.index[i];
            if (i4 % iFloor == 1) {

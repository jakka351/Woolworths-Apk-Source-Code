...
public class PrivateKeyFactory {
    ...
    public static AsymmetricKeyParameter createKey(PrivateKeyInfo privateKeyInfo) throws IOException {
        ...
        if (algorithm.on(BCObjectIdentifiers.sphincsPlus)) {
            SPHINCSPLUSPrivateKey sPHINCSPLUSPrivateKey = SPHINCSPLUSPrivateKey.getInstance(privateKeyInfo.parsePrivateKey());
        ...
        if (algorithm.on(BCObjectIdentifiers.pqc_kem_kyber)) {
            KyberPrivateKey kyberPrivateKey = KyberPrivateKey.getInstance(privateKeyInfo.parsePrivateKey());
            KyberParameters kyberParametersKyberParamsLookup = Utils.kyberParamsLookup(algorithm);
            KyberPublicKey publicKey2 = kyberPrivateKey.getPublicKey();
            byte[] s = kyberPrivateKey.getS();
            byte[] hpk = kyberPrivateKey.getHpk();
            byte[] nonce = kyberPrivateKey.getNonce();
            return publicKey2 != null ? new KyberPrivateKeyParameters(kyberParametersKyberParamsLookup, s, hpk, nonce, publicKey2.getT(), publicKey2.getRho()) : new KyberPrivateKeyParameters(kyberParametersKyberParamsLookup, s, hpk, nonce, null, null);
        }
        if (algorithm.on(BCObjectIdentifiers.pqc_kem_ntrulprime)) {
            ...
            return new NTRULPRimePrivateKeyParameters(Utils.ntrulprimeParamsLookup(algorithm), a.D(aSN1Sequence, 0), a.D(aSN1Sequence, 1), a.D(aSN1Sequence, 2), a.D(aSN1Sequence, 3));
        }
        if (algorithm.on(BCObjectIdentifiers.pqc_kem_sntruprime)) {
            ...
            return new SNTRUPrimePrivateKeyParameters(Utils.sntruprimeParamsLookup(algorithm), a.D(aSN1Sequence2, 0), a.D(aSN1Sequence2, 1), a.D(aSN1Sequence2, 2), a.D(aSN1Sequence2, 3), a.D(aSN1Sequence2, 4));
        }
        if (algorithm.equals((ASN1Primitive) BCObjectIdentifiers.dilithium2) || algorithm.equals((ASN1Primitive) BCObjectIdentifiers.dilithium3) || algorithm.equals((ASN1Primitive) BCObjectIdentifiers.dilithium5) || algorithm.equals((ASN1Primitive) BCObjectIdentifiers.dilithium2_aes) || algorithm.equals((ASN1Primitive) BCObjectIdentifiers.dilithium3_aes) || algorithm.equals((ASN1Primitive) BCObjectIdentifiers.dilithium5_aes)) {
            ...
            ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(privateKey);
            int iB = a.b(aSN1Sequence3, 0);
            if (iB != 0) {
                throw new IOException(YU.a.d(iB, "unknown private key version: "));
            }

...
class PicnicEngine {
    ...
    */
    ...
    private int deserializeSignature(Signature signature, byte[] bArr, int i, int i2) {
        ...
        int iC = (((this.seedSizeBytes * 2) + this.andSizeBytes + this.digestSizeBytes) * i4) + iNumBytes + 32 + i3;
        if (this.transform == 1) {
            iC = (this.UnruhGWithoutInputBytes * iCountNonZeroChallenges) + a.c(i4, iCountNonZeroChallenges, this.UnruhGWithInputBytes, iC);
        }
        if (i != iC) {
            LOG.fine("sigBytesLen = " + i + ", expected bytesRequired = " + iC);
            return -1;
        }
    ...
    private int deserializeSignature2(Signature2 signature2, byte[] bArr, int i, int i2) {
        ...
        for (int i9 = 0; i9 < this.numMPCRounds; i9++) {
            if (contains(signature2.challengeC, this.numOpenedRounds, i9)) {
                ...
                i8 = i8 + iRevealSeedsSize2 + this.stateSizeBytes + this.andSizeBytes + this.digestSizeBytes;
            }
        }
        if (i == i8) {
            ...
            for (int i14 = 0; i14 < this.numMPCRounds; i14++) {
                if (contains(signature2.challengeC, this.numOpenedRounds, i14)) {
                    ...
                    System.arraycopy(bArr, i13, bArr4, 0, iRevealSeedsSize2);
    ...
    private int mpc_AND(int i, int i2, int i3, int i4, Tape tape, Msg msg) {
        ...
        int i5 = msg.unopened;
        if (i5 >= 0) {
            iExtend = Utils.setBit(iExtend, msg.unopened, Utils.getBit(msg.msgs[i5], msg.pos));
        }
    ...
    private void mpc_LowMC(Tape tape, View[] viewArr, int[] iArr, int[] iArr2) {
        ...
        for (int i3 = 1; i3 <= this.numRounds; i3++) {
            ...
            for (int i4 = 0; i4 < 3; i4++) {
                matrix_mul_offset(iArr3, this.stateSizeWords * i4, viewArr[i4].inputShare, 0, kMatricesWithPointerKMatrix2.getData(), kMatricesWithPointerKMatrix2.getMatrixPointer());
            }
            ...
            mpc_matrix_mul(iArr3, i6 * 3, iArr2, i6 * 3, kMatricesWithPointerLMatrix.getData(), kMatricesWithPointerLMatrix.getMatrixPointer(), 3);
    ...
    private void mpc_sbox(int[] iArr, int[] iArr2, Tape tape, Msg msg) {
        for (int i = 0; i < this.numSboxes * 3; i += 3) {
            ...
            int iMpc_AND3 = mpc_AND(bitFromWordArray3, bitFromWordArray, i6, i3, tape, msg);
            int i7 = bitFromWordArray ^ iMpc_AND2;
            int i8 = bitFromWordArray ^ bitFromWordArray2;
            Utils.setBitInWordArray(iArr, i2, i7);
            Utils.setBitInWordArray(iArr, i4, iMpc_AND3 ^ i8);
            Utils.setBitInWordArray(iArr, i, (i8 ^ bitFromWordArray3) ^ iMpc_AND);
        }
    }
    ...
    private int sign_picnic1(int[] iArr, int[] iArr2, int[] iArr3, byte[] bArr, Signature signature) {
        ...
        System.arraycopy(bArrComputeSeeds, picnicEngine.numMPCRounds * i4, signature.salt, 0, 32);
        Tape tape = new Tape(picnicEngine);
        int i5 = picnicEngine.stateSizeBytes;
        int iMax = Math.max(i5 * 9, i5 + picnicEngine.andSizeBytes);
        ...
        while (i6 < picnicEngine.numMPCRounds) {
            ...
            while (i7 < i) {
                ...
                byte[] bArr7 = bArr5;
                if (!picnicEngine.createRandomTape(bArr6, (picnicEngine.seedSizeBytes * i7) + (i4 * i6), signature.salt, i6, i7, bArr5, picnicEngine.stateSizeBytes + picnicEngine.andSizeBytes)) {
            ...
            int i15 = i6;
            if (!picnicEngine.createRandomTape(bArr9, (picnicEngine.seedSizeBytes * 2) + i14, signature.salt, i6, 2, tape3.tapes[i12], picnicEngine.andSizeBytes)) {
                LOG.fine("createRandomTape failed");
                return -1;
            }
            View[] viewArr3 = viewArr2[i15];
            picnicEngine.xor_three(viewArr3[i12].inputShare, iArr, viewArr3[0].inputShare, viewArr3[i13].inputShare);
            tape3.pos = 0;
            int[] iArrLittleEndianToInt = Pack.littleEndianToInt(bArr8, 0, i11 / 4);
            picnicEngine.mpc_LowMC(tape3, viewArr2[i15], iArr3, iArrLittleEndianToInt);
            Pack.intToLittleEndian(iArrLittleEndianToInt, bArr8, 0);
            int[] iArr6 = new int[16];
            View[] viewArr4 = viewArr2[i15];
            picnicEngine.xor_three(iArr6, viewArr4[0].outputShare, viewArr4[i13].outputShare, viewArr4[i12].outputShare);
            ...
            picnicEngine.Commit(bArr9, i14, viewArr2[i15][0], bArr3[i15][0]);
            picnicEngine.Commit(bArr9, i14 + picnicEngine.seedSizeBytes, viewArr2[i15][i13], bArr3[i15][i13]);
            picnicEngine.Commit(bArr9, (picnicEngine.seedSizeBytes * 2) + i14, viewArr2[i15][i12], bArr3[i15][i12]);
            if (picnicEngine.transform == i13) {
                picnicEngine.G(0, bArr9, i14, viewArr2[i15][0], bArr4[i15][0]);
                picnicEngine.G(1, bArr9, i14 + picnicEngine.seedSizeBytes, viewArr2[i15][i13], bArr4[i15][i13]);
                picnicEngine.G(2, bArr9, (picnicEngine.seedSizeBytes * 2) + i14, viewArr2[i15][i12], bArr4[i15][i12]);
                bArr2 = bArr9;
            } else {
                bArr2 = bArr9;
            }
        ...
        while (i16 < picnicEngine2.numMPCRounds) {
            byte[] bArr11 = bArr10;
            picnicEngine2.prove(signature.proofs[i16], picnicEngine2.getChallenge(signature.challengeBits, i16), bArr11, i4 * i16, viewArr5[i16], bArr3[i16], picnicEngine2.transform != 1 ? null : bArr4[i16]);
            i16++;
            picnicEngine2 = this;
            bArr10 = bArr11;
        }
    ...
    private boolean sign_picnic3(int[] iArr, int[] iArr2, int[] iArr3, byte[] bArr, Signature2 signature2) {
        ...
        byte[] bArrCopyOfRange = Arrays.copyOfRange(bArr2, 32, i6);
        ...
        while (true) {
            ...
            if (i13 >= i14) {
                ...
                while (i20 < this.numMPCRounds) {
                    if (contains(signature2.challengeC, this.numOpenedRounds, i20)) {
                        ...
                        int iIndexOf = indexOf(signature2.challengeC, this.numOpenedRounds, i20);
                        ...
                        System.arraycopy(msgArr[i20].msgs[signature2.challengeP[iIndexOf]], i4, signature2.proofs[i20].msgs, i4, this.andSizeBytes);
                        System.arraycopy(bArr5[i20][signature2.challengeP[iIndexOf]], i4, signature2.proofs[i20].C, i4, this.digestSizeBytes);
                    } else {
                        i4 = i17;
                    }
                    i20++;
            ...
            int[] iArrLittleEndianToInt = Pack.littleEndianToInt(bArr3[i13], 0, this.stateSizeWords);
            xor_array(iArrLittleEndianToInt, iArrLittleEndianToInt, iArr, 0);
            int i23 = i13;
            if (simulateOnline(iArrLittleEndianToInt, tapeArr[i23], iArr5, msgArr[i23], iArr3, iArr2) != 0) {
                LOG.fine("MPC simulation failed, aborting signature");
                return false;
            }
            Pack.intToLittleEndian(iArrLittleEndianToInt, bArr3[i23], 0);
            i13 = i23 + 1;
        }
    }
        ...
        while (true) {
            if (i3 < picnicEngine.numMPCRounds) {
                if (picnicEngine.contains(signature2.challengeC, picnicEngine.numOpenedRounds, i3)) {
                    ...
                    int i4 = i3;
                    int iReconstructSeeds = tree3.reconstructSeeds(iArr4, 1, proof2.seedInfo, proof2.seedInfoLen, signature2.salt, i4);
                    i = i4;
                ...
                while (true) {
                    ...
                    if (i11 < i12) {
                        ...
                        if (tree.addMerkleNodes(picnicEngine.getMissingLeavesList(signature2.challengeC), i12 - picnicEngine.numOpenedRounds, signature2.cvInfo, signature2.cvInfoLen) != 0 || (iVerifyMerkleTree = tree.verifyMerkleTree(bArr4, signature2.salt)) != 0) {
                            return -1;
                        }
    ...
    public void mpc_AND_verify(int[] iArr, int[] iArr2, int[] iArr3, Tape tape, View view, View view2) {
        ...
        int i2 = iArr[1];
        int i3 = iArr2[0];
        int i4 = ((((i2 & i3) ^ (iArr2[1] & i)) ^ (i & i3)) ^ bit) ^ bit2;
        iArr3[0] = i4;
    ...
    public void mpc_LowMC_verify(View view, View view2, Tape tape, int[] iArr, int[] iArr2, int i) {
        ...
        for (int i2 = 1; i2 <= this.numRounds; i2++) {
            ...
            iArr3 = iArr;
            mpc_xor_constant_verify(iArr3, kMatricesWithPointerRConstant.getData(), kMatricesWithPointerRConstant.getMatrixPointer(), this.stateSizeWords, i);
            mpc_xor(iArr3, iArr3, 2);
        }
    ...
    public int serializeSignature(Signature signature, byte[] bArr, int i) {
        ...
        int i3 = (((this.seedSizeBytes * 2) + this.stateSizeBytes + this.andSizeBytes + this.digestSizeBytes) * i2) + iNumBytes;
        if (this.transform == 1) {

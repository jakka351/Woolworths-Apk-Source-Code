...
public class OpenBSDBCrypt {
    ...
    private static String createBcryptString(String str, byte[] bArr, byte[] bArr2, int i) {
        if (!allowedVersions.contains(str)) {
            throw new IllegalArgumentException(a.h("Version ", str, " is not accepted by this implementation."));
        }
        StringBuilder sb = new StringBuilder(60);
        sb.append('$');
        sb.append(str);
        sb.append('$');
        sb.append(i < 10 ? a.d(i, "0") : Integer.toString(i));
        sb.append('$');
        encodeData(sb, bArr2);
    ...
    private static byte[] decodeSaltString(String str) {
        ...
        if (charArray.length != 22) {
            throw new DataLengthException(a.m(new StringBuilder("Invalid base64 salt length: "), charArray.length, " , 22 required."));
        }
        for (char c : charArray) {
            if (c > 'z' || c < '.' || (c > '9' && c < 'A')) {
                throw new IllegalArgumentException(a.d(c, "Salt string contains invalid character: "));
            }
        }
    ...
    private static boolean doCheckPassword(String str, byte[] bArr) throws NumberFormatException {
        ...
        if (length != 60 && (length != 59 || str.charAt(2) != '$')) {
            throw new DataLengthException(a.e(length, "Bcrypt String length: ", ", 60 required."));
        }
        int i = 3;
        if (str.charAt(2) == '$') {
        ...
        if (!allowedVersions.contains(strSubstring)) {
            throw new IllegalArgumentException(a.h("Bcrypt version '", strSubstring, "' is not supported by this implementation"));
        }
        String strSubstring2 = str.substring(i, i + 2);
        try {
            int i2 = Integer.parseInt(strSubstring2);
            if (i2 < 4 || i2 > 31) {
                throw new IllegalArgumentException(a.e(i2, "Invalid cost factor: ", ", 4 < cost < 31 expected."));
            }
            return Strings.constantTimeAreEqual(str, doGenerate(strSubstring, bArr, decodeSaltString(str.substring(str.lastIndexOf(36) + 1, length - 31)), i2));
        } catch (NumberFormatException unused) {
            throw new IllegalArgumentException(a.A("Invalid cost factor: ", strSubstring2));
        }
    }
    ...
    private static void encodeData(StringBuilder sb, byte[] bArr) {
        ...
        if (bArr.length != 24 && bArr.length != 16) {
            throw new DataLengthException(a.m(new StringBuilder("Invalid length: "), bArr.length, ", 24 for key or 16 for salt expected"));
        }
        if (bArr.length == 16) {

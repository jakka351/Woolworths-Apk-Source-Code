...
class DHKEM {
    ...
    public AsymmetricCipherKeyPair DeriveKeyPair(byte[] bArr) throws DataLengthException, IllegalArgumentException {
        if (bArr.length < this.Nsk) {
            throw new IllegalArgumentException(a.m(new StringBuilder("input keying material should have length at least "), this.Nsk, " bytes"));
        }
        byte[] bArrConcatenate = Arrays.concatenate(Strings.toByteArray("KEM"), Pack.shortToBigEndian(this.kemId));
        short s = this.kemId;
        if (s == 32) {
        ...
        switch (s) {
            ...
            case 18:
                ...
                for (int i = 0; i <= 255; i++) {
                    byte[] bArrLabeledExpand = this.hkdf.LabeledExpand(bArrLabeledExtract, bArrConcatenate, "candidate", new byte[]{(byte) i}, this.Nsk);

...
public class SICBlockCipher extends StreamBlockCipher implements SkippingStreamCipher {
    ...
    public void init(boolean z, CipherParameters cipherParameters) throws IllegalArgumentException {
        ...
        if (i < bArrClone.length) {
            throw new IllegalArgumentException(a.m(new StringBuilder("CTR/SIC mode requires IV no greater than: "), this.blockSize, " bytes."));
        }
        int i2 = 8 > i / 2 ? i / 2 : 8;
    ...
    public int processBlock(byte[] bArr, int i, byte[] bArr2, int i2) throws IllegalStateException, DataLengthException {
        ...
        if (this.byteCount != 0) {
            sICBlockCipher = this;
            sICBlockCipher.processBytes(bArr, i, this.blockSize, bArr2, i2);
        } else {
            sICBlockCipher = this;
            int i3 = sICBlockCipher.blockSize;
            if (i + i3 > bArr.length) {
            ...
            for (int i4 = 0; i4 < sICBlockCipher.blockSize; i4++) {
                bArr2[i2 + i4] = (byte) (bArr[i + i4] ^ sICBlockCipher.counterOut[i4]);
            }
            incrementCounter();
        }

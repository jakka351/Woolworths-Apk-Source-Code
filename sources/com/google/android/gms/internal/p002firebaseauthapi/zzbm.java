        ...
        if (zziu.zza.zza()) {
            ...
            for (zzxh.zzb zzbVar : zzxhVar.zze()) {
                if (hashSet.contains(Integer.valueOf(zzbVar.zza()))) {
                    throw new GeneralSecurityException(a.e(zzbVar.zza(), "KeyID ", " is duplicated in the keyset, and Tink is configured to reject such keysets with the flag validateKeysetsOnParsing."));
                }
    ...
    public final /* synthetic */ zzoa zza(int i) {
        if (i >= 0 && i < zza()) {
            ...
            if (zza(zzbsVar.zzc)) {
                ...
                throw new IllegalStateException(a.e(i, "Keyset-Entry at position ", " didn't parse correctly"));
            }
            throw new IllegalStateException(a.e(i, "Keyset-Entry at position ", " has wrong status"));
        }
        throw new IndexOutOfBoundsException(b.i(i, zza(), "Invalid index ", " for keyset of size "));
    }
    ...
    public final void zza(zzby zzbyVar) throws GeneralSecurityException, IOException {
        ...
        for (zzxh.zzb zzbVar : zzxhVarZzd.zze()) {
            if (zzbVar.zzb().zzb() == zzwx.zzb.UNKNOWN_KEYMATERIAL || zzbVar.zzb().zzb() == zzwx.zzb.SYMMETRIC || zzbVar.zzb().zzb() == zzwx.zzb.ASYMMETRIC_PRIVATE) {
                throw new GeneralSecurityException(a.i("keyset contains key material of type ", zzbVar.zzb().zzb().name(), " for type url ", zzbVar.zzb().zzf()));
            }
        }

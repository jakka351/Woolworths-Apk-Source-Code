...
public class TypeToken<T> {
    ...
    public TypeToken() {
        ...
        this.rawType = C$Gson$Types.f(typeTokenTypeArgument);
    ...
    public static TypeToken<?> getParameterized(Type type, Type... typeArr) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            Class<?> clsF = C$Gson$Types.f(type2);
            TypeVariable<Class<T>> typeVariable = typeParameters[i];
            for (Type type3 : typeVariable.getBounds()) {
                if (!C$Gson$Types.f(type3).isAssignableFrom(clsF)) {
    ...
    private TypeToken(Type type) {
        ...
        this.type = typeB;
        this.rawType = C$Gson$Types.f(typeB);
    ...
    private static boolean isAssignableFrom(Type type, ParameterizedType parameterizedType, Map<String, Type> map) {
        ...
        Class clsF = C$Gson$Types.f(type);
        ...
        for (Type type3 : clsF.getGenericInterfaces()) {
            if (isAssignableFrom(type3, parameterizedType, new HashMap(map))) {
                return true;
            }
        }
        return isAssignableFrom(clsF.getGenericSuperclass(), parameterizedType, new HashMap(map));
    }
}

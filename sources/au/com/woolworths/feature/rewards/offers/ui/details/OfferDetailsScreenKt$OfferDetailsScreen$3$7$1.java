package au.com.woolworths.feature.rewards.offers.ui.details;

import au.com.woolworths.feature.rewards.offers.detail.OfferDetailsContract;
import au.com.woolworths.feature.rewards.offers.detail.OfferDetailsViewModel;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.FunctionReferenceImpl;

@Metadata(k = 3, mv = {2, 1, 0}, xi = 48)
/* loaded from: classes3.dex */
final /* synthetic */ class OfferDetailsScreenKt$OfferDetailsScreen$3$7$1 extends FunctionReferenceImpl implements Function0<Unit> {
    @Override // kotlin.jvm.functions.Function0
    public final Object invoke() {
        ((OfferDetailsViewModel) this.receiver).l.f(OfferDetailsContract.Action.NavigateUp.f6302a);
        return Unit.f24250a;
    }
}

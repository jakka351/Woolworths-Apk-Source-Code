package au.com.woolworths.feature.shop.modeselector.model;

import au.com.woolworths.base.shopapp.modules.button.Analytics;
import com.woolworths.shop.modeselector.fragment.ButtonFragment;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;

@Metadata(d1 = {"\u0000\u0002\n\u0000¨\u0006\u0000"}, d2 = {"mode-selector_release"}, k = 2, mv = {2, 1, 0}, xi = 48)
/* loaded from: classes3.dex */
public final class ButtonFragmentAnalyticsExtKt {
    public static final Analytics a(ButtonFragment.Analytics analytics) {
        Intrinsics.h(analytics, "<this>");
        return new Analytics(analytics.f23349a);
    }
}

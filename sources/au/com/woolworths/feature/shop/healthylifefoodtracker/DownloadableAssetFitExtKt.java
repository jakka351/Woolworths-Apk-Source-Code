package au.com.woolworths.feature.shop.healthylifefoodtracker;

import au.com.woolworths.shop.graphql.type.DownloadableAssetFit;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;

@Metadata(d1 = {"\u0000\u0002\n\u0000¨\u0006\u0000"}, d2 = {"healthy-life-food-tracker_release"}, k = 2, mv = {2, 1, 0}, xi = 48)
/* loaded from: classes3.dex */
public final class DownloadableAssetFitExtKt {

    @Metadata(k = 3, mv = {2, 1, 0}, xi = 48)
    public /* synthetic */ class WhenMappings {
        static {
            int[] iArr = new int[DownloadableAssetFit.values().length];
            try {
                iArr[1] = 1;
            } catch (NoSuchFieldError unused) {
            }
            try {
                DownloadableAssetFit.Companion companion = DownloadableAssetFit.e;
                iArr[0] = 2;
            } catch (NoSuchFieldError unused2) {
            }
        }
    }

    public static final au.com.woolworths.android.onesite.data.DownloadableAssetFit a(DownloadableAssetFit downloadableAssetFit) {
        Intrinsics.h(downloadableAssetFit, "<this>");
        int iOrdinal = downloadableAssetFit.ordinal();
        if (iOrdinal != 0 && iOrdinal == 1) {
            return au.com.woolworths.android.onesite.data.DownloadableAssetFit.CENTER;
        }
        return au.com.woolworths.android.onesite.data.DownloadableAssetFit.ASPECT_FILL;
    }
}

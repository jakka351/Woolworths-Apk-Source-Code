package au.com.woolworths.shop.productcard.data;

import au.com.woolworths.sdui.common.corecolortheme.CompatibleColorData;
import au.com.woolworths.sdui.shop.fragment.CompatibleColor;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;

@Metadata(d1 = {"\u0000\u0002\n\u0000¨\u0006\u0000"}, d2 = {"productcard-data_release"}, k = 2, mv = {2, 1, 0}, xi = 48)
/* loaded from: classes4.dex */
public final class CompatibleColorExtKt {
    public static final CompatibleColorData a(CompatibleColor compatibleColor) {
        Intrinsics.h(compatibleColor, "<this>");
        return new CompatibleColorData(compatibleColor.f10042a);
    }
}
